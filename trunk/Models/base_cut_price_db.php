...
class base_cut_price_db {
	//Db
	private $db = NULL;
	private $table = 'base_cut_price';
	//数据结果
	...
	public function __construct(&$db){
		$this->db = $db;
		$this->table = DB_PREFIX . 'base_cut_price';
	}
	...
	public function getlist($param){
		...
		$sql_t = "(SELECT a.*,a.goodsName as agname,b.goodsName FROM t_base_cut_price as a 
			LEFT JOIN t_goods_list as b on a.goodsId=b.id
			WHERE a.goodsType=1 " .$where. ") UNION (SELECT a.*,a.goodsName as agname,b.goodsName FROM t_base_cut_price as a 
		...
		$sql .= "
			(SELECT a.*,a.goodsName as agname,b.goodsName FROM t_base_cut_price as a 
			LEFT JOIN t_goods_list as b on a.goodsId=b.id
			WHERE a.goodsType=1 ".$where.") UNION (SELECT a.*,a.goodsName as agname,b.goodsName FROM t_base_cut_price as a 
			LEFT JOIN t_pre_goods_list as b on a.goodsId=b.id
	...
	public function getlistinfo($param){
		if(!empty($param['id'])){
			...
			if ($param['goodsType'] == '1') {
				$sql = "
					SELECT a.*,d.providerName,e.photoPath,b.preferentialPrice as bpreferentialPrice,f.stockNum as fNum 
					FROM t_base_cut_price as a
					LEFT JOIN t_goods_normsvalue_stock as b on a.goodsId=b.goodsId
					LEFT JOIN t_goods_list as c ON c.id=a.goodsId
					LEFT JOIN t_base_provider_ref h on c.providerId=h.id
					LEFT JOIN t_base_enterprise_info d ON d.id = h.refId
				...
				$sql = "
					...
					WHERE a.goodsType=0 and h.providerType=1
				".$where;
			}
	...
	public function getGoodsInfoListByPage($param){
		...
		$sql.="SELECT a.id, a.goodsName,
					...
					INNER JOIN t_base_provider_ref h on a.providerId=h.id
	...
	public function getPreGoodsInfoListByPage($param){
		...
		$sql.="SELECT a.id, a.goodsName,
					...
					WHERE
						a.status=1 and c.status>0 and h.providerType=1";
		if(!empty($param['ids'])){
			$sql.=" and a.id not in({$param['ids']})";
		}
	...
	public function queryGoods($param){
		...
		from t_base_cut_price a
		INNER JOIN t_goods_list b on b.id=a.goodsId
		INNER JOIN t_base_provider_ref h on b.providerId=h.id
