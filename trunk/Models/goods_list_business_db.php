...
class goods_list_business_db{
	...
	public function getGoodsByProvider($param){
		...
		$data  = $this->db->select("select a.id,a.goodsName,
													    	...
													    	b.providerName,c.className,d.originalPrice,d.distPrice,d.preferentialPrice,f.normsValueIds
																from ".$this->t_table." a 
          ...
          foreach (array_reverse($postsizes) as $key => $value) {
              ...
              if (!empty($value['goodsId']) && !empty($value['normsvalue_a_val']) && !empty($value['normsvalue_b_val']))  //二个规格
              {
                  ...
                  if($hasdata[0]['id']){
                    	...
                    	$param_update = " b.stockNum = ".$value['stockNum'].",  a.weight = ".$value['weight'].",   a.preferentialPrice = ".$value['preferentialPrice'];
  ...
  public function findGoodsCode($goodsCode){
  	$sql = "select * from t_goods_list where goodsCode=".$goodsCode;
  	$rest = $this->db->select($sql);
  	if($rest){
  		$this->result['status'] = 1;
  	}else{
  		$this->result['status'] = 0;
