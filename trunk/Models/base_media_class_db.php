...
class base_media_class_db {
	//Db
	private $db = NULL;
	//database table
	private $table = 'base_media_class';
	//数据结果
	...
	public function __construct(&$db){
		$this->db = $db;
		$this->table = DB_PREFIX . 'base_media_class';
	}
	...
	public function getlist($param){
		...
		if(!empty($param['key']) && !empty($param['key_type'])){
			if( $param['key_type'] == 'classTitle' )
			{
				$where['like']['a.classTitle']  = $param['key'];
			}
		...
		$data  = $this->db->select($this->table.' a left join '.$this->table.' b on b.id=a.parentId ','b.classTitle as parentClassTitle, a.id, a.classTitle, a.parentId, a.mark, a.createDate ', $where);
		if(!empty($data))
		{
