	...
	<form class="sui-form" id="search">
      ...
      <span class="sui-dropdown dropdown-bordered select downlist_type"><span class="dropdown-inner"><a id="select" role="button" href="javascript:void(0);" data-toggle="dropdown" class="dropdown-toggle">
        ...
        <ul role="menu" aria-labelledby="drop2" class="sui-dropdown-menu">
          ...
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="tag">标识符</a></li>
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="adName">广告名称</a></li>
			...
			<button type="submit" id="search-bn" data-url="base_adposition_config/index" class="sui-btn btn-large btn-primary">查询</button>
			<?php echo form_a_auth(array('content'=>'添加广告位','check'=>'base_adposition_config/add','class'=>'btn-large','url'=>APP_URL.'base_adposition_config/edit?ref='.$ref));?>
		</div>
	</form>
</div>
<div class="base_adposition_config-list">
</div>
...
_pagedata.url = "base_adposition_config/ajaxIndex?key_type="+key_type+"&key="+key+"&pagesize=<?php echo $page['pagesize'];?>&page=";
_pagedata.container = '.base_adposition_config-list';
