<ul class="sui-nav nav-tabs nav-xlarge">
  <li  onclick="s_navclick('index');"><a>转让列表</a></li>
  <?php if (auth_check_permissions('goods_transfer_class/index')):?>
    <li  class="active" onclick="s_navclick('goods_transfer_class');"><a>转让分类</a></li>
  <?php endif;?>
</ul>
<div class="content-top">
  <form class="sui-form" id="search">
    <div class="controls">
      ...
      <button type="submit" id="search-bn" data-url="goods_transfer_class/index" class="sui-btn btn-large btn-primary">查询</button>
      <?php echo form_a_auth(array('content'=>'添加','class'=>'btn-large','check'=>'goods_transfer_class/add','url'=>APP_URL.'goods_transfer_class/edit?ref='.$ref));?>
      <?php /*echo form_a_auth(array('content'=>'更新分类缓存','class'=>'btn-large','onclick'=>'updatecahce();','check'=>'goods_transfer_class/edit_cache'));*/?>
    </div>
  </form>
</div>
<div class="goods_transfer_class-list">
</div>
...
_pagedata.url = "goods_transfer_class/ajaxIndex?key_type="+key_type+"&key="+key+"&pagesize=<?php echo $page['pagesize'];?>&page=";
_pagedata.container = '.goods_transfer_class-list';
  ...
  $.confirm({
    ...
    okHide: function() {
      $.ajax({
          type    : "post",
          async   : false,
          url     : '<?php echo APP_URL;?>goods_transfer_class/updatecahce',
          data    : '',
          dataType: 'json',
