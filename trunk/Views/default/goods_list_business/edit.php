...
<form id="goods_list_business-form" class="sui-form form-horizontal sui-validate" novalidate="novalidate">
...
$(function(){
  ...
  $(".js_sendAddress").show();
  var _opts = {};
  _opts.url = OO._SRVPATH + 'goods_list_business/save';
  SAYIMO.form.init('#goods_list_business-form', _opts,submitBefore);
});
...
function navclick(action)
{
  ...
  SAYIMO.go_url("<?php echo APP_URL;?>goods_list_business/"+action+"<?php echo $id ? "?id=$id" : '';?>");
}
...
$(".goods_class").on('mouseenter','.classA',function(){
  var _cur = $(this);
    ...
    $.ajax({
      ...
      data:{'id':_cur.find('a').attr('value'),'mallType':4},
      dataType:"json",
      async:false,
...
$(".goods_class").on('click','li a',function(){
  var _cur = $(this).eq(0);
  var _has_submenu = $(this).eq(0).parent("li.dropdown-submenu").hasClass("dropdown-submenu");
  if(!_has_submenu){//含有子类不能点击
...
$(function(){
  ...
  SAYIMO.dialogView('js_add_company','供应商','providerName','admin_provider/viewProvider?providerType=2&lm=1','js_btn_item',company_callback);
});
