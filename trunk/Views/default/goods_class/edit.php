...
<ul class="sui-breadcrumb">
	<li><a><?php echo ($id)?'修改':'添加';?>商品分类</a></li>
...
<form id="goods_class-form" class="sui-form form-horizontal sui-validate" novalidate="novalidate">
  ...
  <div class="control-group followChkBtn">
    ...
    <div class="controls type_chk">
      ...
      <label class="checkbox-pretty inline <?php if(strrpos($attr['mallType'],'4')>-1){echo "checked";}?>">
        <input type="checkbox" <?php if(strrpos($attr['mallType'],'4')>-1){echo 'checked="checked"';}?> name="type_chk" value="4" data-rules="required"><span>联盟商商城</span>
      </label>
...
$(function(){
  var _opts = {};
  _opts.url = OO._SRVPATH + 'goods_class/save';
  SAYIMO.form.init('#goods_class-form', _opts);
});
//隐藏type跟踪多选table
$("#goods_class-form").on('click', ".followChkBtn", function(){
  setTimeout(function() {
    var type = [];    //帐号类型
    ...
    $('#parentId').val('');
    $('#change_mallType').html('<i class="caret"></i>请选择');
  }, 300);
})
...
$(".goods_class").on('mouseenter','.classA',function(){
  ...
  var mallType = $("#mallType").val();
    ...
    $.ajax({
      ...
      data:{'id':_cur.find('a').attr('value'),'mallType':mallType},
      dataType:"json",
      async:false,
      success:function(data){
        var ul = '<ul class="sui-dropdown-menu">';
        for (var i = data.length - 1; i >= 0; i--) {
          ul += '<li role="presentation" '+ (data[i].has_sub?'class="dropdown-submenu  classB"':'') +'><a role="menuitem" tabindex="-1" href="javascript:void(0);" classLevelId="2" value="'+data[i].id+'">'+(data[i].has_sub?'<i class="sui-icon icon-angle-right pull-right"></i>':'')+data[i].className+'</a></li>\n';
        }
