<ul class="sui-nav nav-tabs nav-xlarge" style="margin-top: 8px;">
  <li class="<?php echo $orderType==3?'active':''?>" onclick="s_navclick('index?orderType=3');"><a>充值记录</a></li>
  <li onclick="s_navclick('indexrecord');"><a>消费记录</a></li>
  <li onclick="s_navclick('indexdetail');"><a>收入明细</a></li>
  <li class="<?php echo $orderType==7?'active':''?>"onclick="s_navclick('index?orderType=7');"><a>提现记录</a></li>
</ul>
	...
	<form class="sui-form" id="search">
     ...
     <span class="sui-dropdown dropdown-bordered select downlist_type"><span class="dropdown-inner"><a id="select" role="button" href="javascript:void(0);" data-toggle="dropdown" class="dropdown-toggle">
      ...
      <span class="dropdown-inner">
       ...
       <input type="hidden" value="<?php echo $orderType?>" id="orderType" name="orderType" />
    <button type="submit" id="search-bn" data-url="<?php echo APP_URL;?>base_recharge/index" class="sui-btn btn-large btn-primary">查询</button>
		</div>
	</form>
</div>
...
_pagedata.url = "base_recharge/ajaxIndex?key_type="+key_type+"&key="+key+"&orderType="+orderType+"&pagesize=<?php echo $page['pagesize'];?>&page=";
