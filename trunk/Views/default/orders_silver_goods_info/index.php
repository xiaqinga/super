...
<ul class="sui-nav nav-tabs nav-xlarge" style="margin-top: 8px;">
  ...
  <?php if (auth_check_permissions('orders_silver_goods_info/tobe_paid')):?>
  <li <?php echo $status == '1' ? 'class="active"' : '';?> onclick="navclick(1);"><a>待支付</a></li>
  <?php endif;?>
  <?php if (auth_check_permissions('orders_silver_goods_info/tobe_shipped')):?>
  <li <?php echo $status == '2' ? 'class="active"' : '';?> onclick="navclick(2);"><a>待发货</a></li>
  <?php endif;?>
  <?php if (auth_check_permissions('orders_silver_goods_info/already_shipped')):?>
  <li <?php echo $status == '3' ? 'class="active"' : '';?> onclick="navclick(3);"><a>已发货</a></li>
  <?php endif;?>
  <?php if (auth_check_permissions('orders_silver_goods_info/finish')):?>
...
<div class="content-top">
  <form class="sui-form" id="search">
    <div class="controls">
      ...
      <button type="button" id="search-bn" data-url="orders_silver_goods_info/index" onclick="search()" class="sui-btn btn-large btn-primary">查询</button>
      <?php
        if($status == 2):
      ?>
       
        <?php echo form_a_auth(array('content'=>'批量发货','class'=>'btn-large','onclick'=>'sendGoods()','check'=>'orders_silver_goods_info/bulk_shipment'));?>
      <?php 
        endif;
      ?>
      <?php
      ...
      ?>
        ...
        <?php echo form_a_auth(array('content'=>'导出订单','class'=>'btn-large','onclick'=>'exportSendGoods()','check'=>'orders_silver_goods_info/export_order'));?>
      <?php 
        endif;
      ?>
...
<div class="orders_silver_goods_info-list">
</div>
...
function search() {
    ...
    SAYIMO.go_url("<?php echo APP_URL;?>orders_silver_goods_info/index?" + key_type + "=" + encodeURIComponent(key) + "&status=" + status+"&page="+page); 
}
...
_pagedata.url = "orders_silver_goods_info/ajaxIndex?" + key_type + "=" + encodeURIComponent(key) + "&status=" + status +"&pagesize=<?php echo $page['pagesize'];?>&page=";
_pagedata.container = '.orders_silver_goods_info-list';
...
<script type="text/javascript">
  ...
  {
    if(status){
      SAYIMO.go_url(encodeURI("<?php echo APP_URL;?>orders_silver_goods_info/index?status=" + status));
    }else{
      SAYIMO.go_url(encodeURI("<?php echo APP_URL;?>orders_silver_goods_info/index?"));
    }
  }
  ...
  function exportSendGoods(){
    ...
    window.open("<?php echo APP_URL?>orders_silver_goods_info/exportorder?providerId=" + providerId + "&status=" + status + "&ids=" + ids + "&chkStartDate=" + chkStartDate + "&chkEndDate=" + chkEndDate + "&" + key_type + "=" + encodeURIComponent(key));
  }
  //时间
