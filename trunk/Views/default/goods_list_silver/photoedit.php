...
$(document).ready(function(){
    if(!$("#id").val()){
      $.alert({
        ...
        body: '亲，先完善商品基本信息',
        okHidden: function(e){
          SAYIMO.go_url("<?php echo APP_URL;?>goods_list_silver/edit");
        }
      })
    }
});
...
function navclick(action)
{
  ...
  SAYIMO.go_url("<?php echo APP_URL;?>goods_list_silver/"+action+"?id="+$("#id").val());
}
...
function photo_save()
{
  ...
  $.ajax({
    type    : "post",
    async   : false,
    url     : '<?php echo APP_URL."goods_list_silver/photosave";?>',
    ...
    success : function (data){
      if (data['status'] == true){
        ...
        $.alert({
          ...
          body: '恭喜亲，图片保存成功',
          okHide: function(e){
            SAYIMO.go_url("<?php echo APP_URL;?>goods_list_silver/normsEdit?id="+$("#id").val());
          }
        })
      }else{
        $.alert({
  ...
  $.confirm({
    ...
    okHidden: function() {
      ...
      $.ajax({
        type    : "post",
        async   : false,
        url     : '<?php echo APP_URL."goods_list_silver/photodelete";?>',
        ...
        success : function (data){
          ...
          if (data['data']['status'] == 1){
            $.alert(data['data']['msg']);
            SAYIMO.go_url("<?php echo APP_URL;?>goods_list_silver/photoEdit?id="+$("#id").val());
          }else{
            $.alert(data['data']['msg']);
          }
        }
