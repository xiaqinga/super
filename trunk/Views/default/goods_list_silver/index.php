      ...
      <span class="sui-dropdown dropdown-bordered select downlist_status"><span class="dropdown-inner"><a id="select" role="button" href="javascript:void(0);" data-toggle="dropdown" class="dropdown-toggle">
        ...
        <ul role="menu" aria-labelledby="drop2" class="sui-dropdown-menu">
          ...
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="4">已驳回</a></li>
			...
			<button type="submit" id="search-bn" data-url="<?php echo APP_URL.'goods_list_silver/index';?>" class="sui-btn btn-large btn-primary">查询</button>
			<?php echo form_a_auth(array('content'=>'添加商品','check'=>'goods_list_silver/add','class'=>'btn-large','url'=>APP_URL.'goods_list_silver/edit?ref='.$ref));?>
      <?php echo form_a_auth(array('content'=>'导出商品','check'=>'goods_list_silver/export_order','class'=>'btn-large','onclick'=>'js_export()'));?>
      <!-- <a href="javascript:void(0);" onclick="js_export();" class="sui-btn btn-large">导出商品</a> -->
     	</div>
	</form>
</div>
<div class="goods_list_silver-list">
</div>
...
_pagedata.url = "goods_list_silver/ajaxIndex?goodsCode="+encodeURIComponent(goodsCode)+"&goodsName="+encodeURIComponent(goodsName)+"&goodsClassId="+goodsClassId+"&providerId="+providerId+"&status="+status+"&className="+className+"&providerName="+encodeURIComponent(providerName)+"&pagesize=<?php echo $page['pagesize'];?>&page=";
_pagedata.container = '.goods_list_silver-list';
...
function js_export(){
  var providerId = $("#providerId").val();
  if(providerId){
    window.open("<?php echo APP_URL?>goods_list_silver/exportorder?providerId=" + providerId);
  }else{
    $.alert("请选择供应商");
  }
}
...
$('#sync').on('click',function(){
    $.ajax({
      url:'<?php echo APP_URL?>goods_list_silver/sync_goods',
      success:function(data){
          $.alert("同步成功");
      }
    });
