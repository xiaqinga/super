...
<form id="goods_list_silver-form" class="sui-form form-horizontal sui-validate" novalidate="novalidate">
...
$(function(){
  ...
  $(".js_sendAddress").show();
  var _opts = {};
  _opts.url = OO._SRVPATH + 'goods_list_silver/save';
  SAYIMO.form.init('#goods_list_silver-form', _opts,submitBefore);
});

//隐藏type跟踪多选table
$("#goods_list_silver-form").on('click', ".followChkBtn", function(){
  setTimeout(function() {
    var type = [];    //帐号类型
...
function navclick(action)
{
  ...
  SAYIMO.go_url("<?php echo APP_URL;?>goods_list_silver/"+action+"<?php echo $id ? "?id=$id" : '';?>");
}
...
$(".goods_class").on('mouseenter','.classA',function(){
  ...
  // var ajaxMallType = $("#mallType").val();
    $.ajax({
      ...
      data:{'id':_cur.find('a').attr('value'),'mallType':2},
      dataType:"json",
      async:false,
...
//隐藏isTurnBack跟踪单选项
$("#goods_list_silver-form").on('click', ".isTurnBackRadio", function(){
  setTimeout(function() {
    var isTurnBack;  
...
//隐藏isFreight跟踪单选项
$("#goods_list_silver-form").on('click', ".isFreightRadio", function(){
  setTimeout(function() {
    var isFreight;  
...
//隐藏status跟踪单选项
$("#goods_list_silver-form").on('click', ".statusRadio", function(){
  setTimeout(function() {
    var status;  
...
$(function(){
  ...
  SAYIMO.dialogView('js_add_company','供应商','providerName','admin_provider/viewProvider?mallType=2','js_btn_item',company_callback);
});
