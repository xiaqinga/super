<ul class="sui-nav nav-tabs nav-xlarge">
    ...
    <li class="active" onclick="s_navclick('Team');"><a>团队列表</a></li>
    ...
    <input type ="hidden" id="gameId" name="gameId" value="<?php echo $gameId?>">
      <span class="sui-dropdown dropdown-bordered select"><span class="dropdown-inner"><a id="select" role="button" href="javascript:void(0);" data-toggle="dropdown" class="dropdown-toggle">
        ...
        <ul role="menu" aria-labelledby="drop2" class="sui-dropdown-menu">
          ...
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="realName">真实姓名</a></li>
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="memberNumRank">排名</a></li>
        </ul></span></span>
      <input type="text" id="key" name="key" class="input-xlarge input-fat" value="<?php echo $key;?>">
      <button type="submit" id="search-bn" data-url="<?php echo APP_URL;?>base_startup_game_detail/Team" class="sui-btn btn-large btn-primary">查询</button>   
    </div>
  </form>
</div>
...
_pagedata.url = "base_startup_game_detail/ajaxIndex?key_type="+key_type+"&key="+key+"&gameId="+gameId+"&pagesize=<?php echo $page['pagesize'];?>&page=";
