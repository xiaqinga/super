...
<form id="goods_photo-form" class="sui-form form-horizontal sui-validate" novalidate="novalidate">
  ...
  <div class="control-group">
    ...
    <div class="controls thumb_photo_controls">
      ...
      <span style="color:#ff9900;">&nbsp;&nbsp;图片尺寸&nbsp;&nbsp;280*280px</span>
    </div>
  </div>
  <div class="control-group">
  ...
  <input type="hidden" value="<?php echo $isGiveScore?>" id="isGiveScore">
</form>
</div>
...
$(document).ready(function(){
    if(!$("#id").val()){
      $.alert({
        ...
        body: '亲，先完善商品基本信息',
        okHidden: function(e){
          SAYIMO.go_url("<?php echo APP_URL;?>goods_list/edit");
        }
      })
    }
});
...
function photo_save()
{
  ...
  $.ajax({
    type    : "post",
    async   : false,
    url     : '<?php echo APP_URL."goods_list/photosave";?>',
    ...
    success : function (data){
      ...
      if (data['status'] == true){
        ...
        $.alert({
          ...
          body: '恭喜亲，图片保存成功',
          okHide: function(e){
            SAYIMO.go_url("<?php echo APP_URL;?>goods_list/normsEdit?id="+$("#id").val()+"&isGiveScore="+$("#isGiveScore").val());
          }
        })
      }else{
        $.alert({
  ...
  $.confirm({
    ...
    okHidden: function() {
      ...
      $.ajax({
        type    : "post",
        async   : false,
        url     : '<?php echo APP_URL."goods_list/photodelete";?>',
        data    : "id=" + delItem.attr('photoId') + "&goodsId=" + $("#id").val(),
        dataType: 'json',
        success : function (data){
          if (data['data']['status'] == 1){
            $.alert(data['data']['msg']);
            SAYIMO.go_url("<?php echo APP_URL;?>goods_list/photoEdit?id="+$("#id").val()+"&isGiveScore="+$("#isGiveScore").val());
          }else{
            $.alert(data['data']['msg']);
          }
        }
...
$(".thumb_photo_controls").on("click", ".Js_upload", function(){
  ...
  $(".Js_upload").J_jcorp({
    ...
    'aspectRatio': 1,//裁剪宽高比
    'maxSize': [280,280],//裁剪最大尺寸 [width,height]
    'minSize': [280,280],//裁剪最小尺寸 [width,height]
    'picSize': [280,280],//最终保存图片尺寸 [width,height]
