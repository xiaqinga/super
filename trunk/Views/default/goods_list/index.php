	...
	<form class="sui-form" id="search">
      ...
      <span class="sui-dropdown dropdown-bordered select downlist_status"><span class="dropdown-inner"><a id="select" role="button" href="javascript:void(0);" data-toggle="dropdown" class="dropdown-toggle">
        ...
        <ul role="menu" aria-labelledby="drop2" class="sui-dropdown-menu">
          ...
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="3">已驳回</a></li>
        </ul></span></span>
			<button type="submit" id="search-bn" data-url="<?php echo APP_URL.'goods_list/index';?>" class="sui-btn btn-large btn-primary">查询</button>
			<?php echo form_a_auth(array('content'=>'添加商品','check'=>'goods_list/add','class'=>'btn-large','url'=>APP_URL.'goods_list/edit?ref='.$ref));?>
      <?php echo form_a_auth(array('content'=>'导出商品','check'=>'goods_list/export_order','class'=>'btn-large','onclick'=>'js_export()'));?>
...
_pagedata.url = "goods_list/ajaxIndex?goodsCode="+encodeURIComponent(goodsCode)+"&goodsName="+encodeURIComponent(goodsName)+"&goodsClassId="+goodsClassId+"&providerId="+providerId+"&status="+status+"&className="+className+"&providerName="+encodeURIComponent(providerName)+"&pagesize=<?php echo $page['pagesize'];?>&page=";
_pagedata.container = '.goods_list-list';
...
function js_export(){
  var providerId = $("#providerId").val();
  if(providerId){
    window.open("<?php echo APP_URL?>goods_list/exportorder?providerId=" + providerId);
  }else{
    $.alert("请选择供应商");
  }
}
...
$('#sync').on('click',function(){
    $.ajax({
      url:'<?php echo APP_URL?>goods_list/sync_goods',
      success:function(data){
          $.alert("同步成功");
      }
    });
