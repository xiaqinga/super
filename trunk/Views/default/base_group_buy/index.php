...
<ul class="sui-nav nav-tabs nav-xlarge">
  ...
  <li class="active" onclick="s_navclick('groupbuy');"><a>团购专区</a></li>
  <?php endif;?>
   ...
   <div class="controls">
      <span class="sui-dropdown dropdown-bordered select"><span class="dropdown-inner"><a role="button" href="javascript:void(0);" data-toggle="dropdown" class="dropdown-toggle">
        ...
        <ul role="menu" aria-labelledby="drop2" class="sui-dropdown-menu">
          ...
          <li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:void(0);" value="price">团购价格</a></li>
        </ul></span></span>
      <input type="<?php echo showKeyInput($key_type);?>" id="key" name="key" class="input-xlarge input-fat" value="<?php echo $key;?>">
      
      <button type="submit" id="search-bn" data-url="<?php echo APP_URL ;?>base_group_buy/index" class="sui-btn btn-large btn-primary">查询</button>
      <?php echo form_a_auth(array('content'=>'添加活动','class'=>'btn-large','url'=>APP_URL.'base_group_buy/edit?ref='.$ref));?>
    </div>
  </form>
</div>
...
_pagedata.url = "base_group_buy/ajaxIndex?key_type="+key_type+"&key="+key+"&pagesize=<?php echo $page['pagesize'];?>&page=";
