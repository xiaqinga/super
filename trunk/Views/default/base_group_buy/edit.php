...
<form id="base_group_buy-form" class="sui-form form-horizontal sui-validate" novalidate="novalidate">
  <div class="control-group">
    ...
    <div class="controls">
      ...
      <input type="hidden" name="identifier" id="identifier" value="<?php echo $attr['identifier'];?>">
  ...
  <div class="control-group">
    <label for="inputDes" class="control-label v-top"><span class="required">*</span>团购价：</label>
    <div class="controls">
      <input min="0" type="number" class="input-xlarge input-xfat" id="price" name="price" data-rules="required" data-empty-msg="团购价不能为空！" value="<?php echo $attr['price']; ?>">（元）
    </div>
  </div>
  <div class="control-group">
    <label for="inputDes" class="control-label v-top"><span class="required">*</span>团购库存：</label>
    <div class="controls">
      <input min="0" type="number" class="input-xlarge input-xfat" id="stockNum" name="stockNum" data-rules="required|number" data-empty-msg="团购库存不能为空！" value="<?php echo $attr['stockNum']; ?>">（件）
    </div>
  </div>
  <div class="control-group">
    <label for="inputDes" class="control-label v-top"><span class="required">*</span>活动时间：</label>
    <div class="controls">
      <input class="input-xlarge input-fat" style="width:180px;" id="startDate" name="startDate" value="<?php echo $attr['startDate'];?>" <?php if(empty($attr['startDate'])){echo "data-toggle='datepicker' data-date-timepicker='true'";}?> data-rules="required">
      &nbsp;至&nbsp;
      <input class="input-xlarge input-fat" style="width:180px;" id="endDate" name="endDate" value="<?php echo $attr['endDate'];?>" <?php if(empty($attr['endDate'])){echo "data-toggle='datepicker' data-date-timepicker='true'";}?> data-rules="required">
    </div>
  </div>
  <div class="control-group">
    <label for="inputDes" class="control-label v-top"><span class="required">*</span>单品限购：</label>
    <div class="controls">
      <input min="0" type="number" class="input-xlarge input-xfat" id="maxBuy" name="maxBuy" data-rules="required" data-empty-msg="单品限购不能为空！" value="<?php echo $attr['maxBuy']; ?>">（件/人   0表示不限购）
    </div>
  </div>
...
{ 
  ...
  SAYIMO.go_url("<?php echo APP_URL;?>base_group_buy/"+url+"?id="+$("#id").val()+"&goodsType="+$("#goodsType").val());
}
...
function save()
{
  putdata = $("#base_group_buy-form").serialize();
  $.ajax({
    type    : "post",
    async   : false,
    url     : '<?php echo APP_URL."base_group_buy/save";?>',
    data    : putdata,
    dataType: 'json',
    success : function (data){
      if(data['id'] && data['goodsType']){
        SAYIMO.go_url('<?php echo APP_URL."base_group_buy/addgoods";?>'+"?id="+data['id']+"&goodsType="+data['goodsType']);
      }else{
        SAYIMO.go_url('<?php echo APP_URL."base_group_buy/addgoods";?>'+"?id="+$("#id").val()+"&goodsType="+$("#goodsType").val());
      }
    }
  })
}
...
$(function(){
  var _opts = {};
  _opts.url = OO._SRVPATH + 'base_group_buy/save';
  SAYIMO.form.init('#base_group_buy-form',_opts);
});
