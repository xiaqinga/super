...
<table class="sui-table table-bordered-simple">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="center">
      	<?php echo form_a_auth(array('content'=>'详情','class'=>'btn-link','url'=>APP_URL.'base_seckill/info?id='.$rs['id'].'&ref='.urlencode($ref).'&goodsType=1','img'=>'check.png'));?>
        <?php
          if($rs['startDate'] < Date('Y-m-d H:i:s') && $rs['endDate'] > Date('Y-m-d H:i:s')){
            
          }else{
            echo form_a_auth(array('content'=>'修改','class'=>'btn-link','url'=>APP_URL.'base_seckill/addGoods?id='.$rs['id'].'&ref='.urlencode($ref).'&goodsType=1','img'=>'update.png'));
