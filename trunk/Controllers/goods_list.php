...
class goods_list extends common {
	...
	public function index()
	{
		...
		$this->view($data,'goods_list/index');
	}
	...
	public function ajaxIndex(){
		...
		$data  = array(
			...
			'className' => $className,
			'providerName' => $providerName,
			'ref' => $this->queryVar('ref' , APP_URL . 'goods_list/index?goodsCode='.$goodsCode.'&goodsName='.$goodsName.'&goodsClassId='.$goodsClassId.'&providerId='.$providerId.'&status='.$status.'&className='.$className.'&providerName='.$providerName.'&page='.$page)
		);
		$this->setActionLog('goods_list','QUERY','查询金商城商品列表');
		$this->view($data,'goods_list/ajaxindex');
	}
  ...
  function exportOrder(){
    ...
    echo "</table>";
    $this->setActionLog('goods_list','QUERY','导出金商城商品列表');
    return (true);  
	...
	public function edit(){
		...
		$data['ref'] = APP_URL . 'goods_list/photoedit?id='.$id;
		$data['goback'] = APP_URL . 'goods_list/index';
		$this->view($data,'goods_list/edit');
	}

	public function photoEdit()
	{
		$id = $this->queryVar('id');
		$isGiveScore=$this->queryVar('isGiveScore');
		if(!empty($id)){
			$param['goodsId'] = $id;
		...
		$data['isGiveScore']=$isGiveScore;
		$data['ref'] = $this->queryVar('ref' , APP_URL . 'goods_list/index');
		$this->view($data,'goods_list/photoedit');
	}
	...
	public function normsEdit()
	{
		...
		$data['isGiveScore']=$isGiveScore;
		$data['ref'] = $this->queryVar('ref' , APP_URL . 'goods_list/index');
		$this->view($data,'goods_list/normsedit');
	}
	...
	public function save(){
		...
		$param['isGiveScore']=$this->queryVar('isGiveScore');
		$param['mallType'] = 1;
		$param['isGiveScore'] = $this->queryVar('isGiveScore');
		...
		if(!$id)
		{
			...
			$id=$resp['data']['id'];
			$ref =  APP_URL . 'goods_list/photoedit?id='.$resp['data']['id'].'&isGiveScore='.$param['isGiveScore'];
			$opt  = '添加';
			if($resp['status']){
				$param_b = array(
					'goodsId' => $resp['data']['id'], 
					'pointsType' => 1, 
					'points' => 0, 
					'pointsPercent' => 0, 
					'status' => 1, 
				...
				$resp_b = $this->model->write('goods_list','create_brokerage', $param_b);
			}
			$this->setActionLog('goods_list','SAVE','添加金商城商品');
		}
		else
		{
			$param['id'] = $id;
			...
			$opt  = '修改';
			$ref =  APP_URL . 'goods_list/photoedit?id='.$id.'&isGiveScore='.$param['isGiveScore'];
			$this->setActionLog('goods_list','UPDATE','修改金商城商品');
		}
			...
			if(count($main_photos)>0){
				foreach ($main_photos as $key => $value) {
					if(!$value['id']){
						...
						$resp = $this->model->write('goods_photo','create', $param);
						if($resp['status']){$ok = true;}
						$this->setActionLog('goods_list','SAVE','添加金商城商品图片');
					}else{
						//更新
						$param = array(
							'id' => $value['id'],
						...
						$resp = $this->model->write('goods_photo','update', $param);
						if($resp['status']){$ok = true;}
						$this->setActionLog('goods_list','UPDATE','修改金商城商品图片');
					}
				}
			}
	...
	public function delete(){
		...
		$this->setActionLog('goods_list','DELETE','删除金商城商品');
		$this->jsonout($resp['status'],array(
	...
	public function upstore(){
		...
		if(!empty($data[1])){
			...
			$this->setActionLog('goods_list','UPDATE','修改金商城商品状态');
			$this->jsonout($resp['status'],array(
	...
	public function photoDelete(){
		...
		$this->setActionLog('goods_list','DELETE','删除金商城商品图片');
		$this->jsonout($resp['status'],array(
			'status'=> 1,
			'msg'=>'图片删除成功',
			'ref'=> $ref
	...
	public function getpartgoods(){
		...
		$data['isGiveScore']=$this->queryVar('isGiveScore');

	    $this->view($data, 'goods_list/partgoods'); //输出
	}
  ...
  public function savesize(){
      ...
      $ret      = $dataitem['status'];
      $this->setActionLog('goods_list','SAVE','更改金商城商品规格');
      if ($ret)
      {
          $data['msg'] = true;
      }
  ...
  public function savesizegoods(){
      ...
      $ret      = $dataitem['status'];
      $this->setActionLog('goods_list','SAVE','保存金商城商品组合商品');
      if ($ret)
      {
          $data['msg'] = true;
					//新增/修改商品搜索引擎
	...
	public function editReject(){
		$data['id'] = $this->queryVar('id');
		$data['mallType'] = $this->queryVar('mallType');
		$this->view($data,'goods_list/editreject');
	}
	...
	public function setReject(){
		...
		if($mallType==1){
			$ref = APP_URL.'goods_list/index';
		}elseif($mallType==2){
			$ref = APP_URL.'goods_list_silver/index';
		}elseif($mallType==4){
			$ref = APP_URL.'goods_list_business/index';
		}
		
		// var_dump($mallType);die();
		$resp = $this->model->write('goods_list','setReject',$param);
		$this->jsonout($resp['status'],array(
			'msg'=>($resp['status']) ? '驳回成功' : '驳回失败',
			'ref'=> $ref
		));
	}
	...
	public function setgoodscode(){
		$param['goodsCode'] = $this->queryVar('goodsCode');
		$param['id'] = $this->queryVar('id');
		$resp = $this->model->read('goods_list','setgoodscode',$param);
		if($resp['status'] == 1){
