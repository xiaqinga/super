...
class goods_list_business extends common {
	...
	public function index()
	{
		...
		$resp = $this->model->read('goods_list_business','getlist',$param);
		...
		$this->view($data,'goods_list_business/index');
	}
	...
	public function ajaxIndex(){
		...
		$data  = array(
			...
			'className' => $className,
			'providerName' => $providerName,
			'ref' => $this->queryVar('ref' , APP_URL . 'goods_list_business/index?goodsCode='.$goodsCode.'&goodsName='.$goodsName.'&goodsClassId='.$goodsClassId.'&providerId='.$providerId.'&status='.$status.'&className='.$className.'&providerName='.$providerName.'&page='.$page)
		);
		$this->setActionLog('goods_list_business','QUERY','查询商企商品列表');
		$this->view($data,'goods_list_business/ajaxindex');
	}
  ...
  function exportOrder(){
	  ...
	  echo "</table>";
    $this->setActionLog('goods_list_business','QUERY','导出商企商品列表');
    return (true);  
	...
	public function edit(){
		...
		$data['ref'] = APP_URL . 'goods_list_business/photoedit?id='.$id;
		$data['goback'] = APP_URL . 'goods_list_business/index';
		$this->view($data,'goods_list_business/edit');
	}
	...
	public function photoEdit()
	{
		...
		$data['ref'] = $this->queryVar('ref' , APP_URL . 'goods_list_business/index');
		$this->view($data,'goods_list_business/photoedit');
	}
	...
	public function normsEdit()
	{
		...
		$data['id'] = $id;
		$data['ref'] = $this->queryVar('ref' , APP_URL . 'goods_list_business/index');
		$this->view($data,'goods_list_business/normsedit');
	}
	...
	public function save(){
		...
		$param['goodsAttribute'] = $this->queryVar('goodsAttribute',0);
		$param['description'] = $this->queryVar('description',0);
		$param['isTurnBack'] = $this->queryVar('isTurnBack','N');
		$param['isFreight'] = $this->queryVar('isFreight','N');
		$param['logisticsCostId'] = $this->queryVar('freightTmplateId',0);
		$param['sendAddress'] = $this->queryVar('sendAddress',0);
		$param['status'] = $this->queryVar('status',1);
		$param['goodsCode'] = $this->queryVar('goodsCode');
		$param['mallType'] = 4;
		//返回URL
		$ref = $this->queryVar('ref',APP_URL . 'goods_list_business/index');
		$ref = urldecode($ref);
		...
		if(!$id)
		{
			...
			$resp = $this->model->write('goods_list_business','create', $param);
			$ref =  APP_URL . 'goods_list_business/photoedit?id='.$resp['data']['id'];
			$opt  = '添加';
			$id=$resp['data']['id'];
			$this->setActionLog('goods_list_business','SAVE','添加商企商品');
		}
		else
		{
			$param['id'] = $id;
			...
			$resp = $this->model->write('goods_list_business','update', $param);
			$opt  = '修改';
			$ref =  APP_URL . 'goods_list_business/photoedit?id='.$id;
			$this->setActionLog('goods_list_business','UPDATE','修改商企商品');
		}
		//新增/修改商品搜索引擎
		try{
			if($resp['status']){
		...
		if($id){
			...
			if(count($thumb_photos)>0){
				if(!$thumb_photos[0]['id']){
					...
					$this->setActionLog('goods_list_business','SAVE','添加商企商品列表图');
				}else{
					//更新
					$param = array(
					...
					if($resp['status']){$ok = true;}
					$this->setActionLog('goods_list_business','UPDATE','修改商企商品列表图');
				}
			}
			//添加/更新商品轮廓图
			if(count($main_photos)>0){
				foreach ($main_photos as $key => $value) {
					if(!$value['id']){
						...
						$resp = $this->model->write('goods_photo','create', $param);
						if($resp['status']){$ok = true;}
						$this->setActionLog('goods_list_business','SAVE','添加商企商品轮播图');
					}else{
						//更新
						$param = array(
							'id' => $value['id'],
						...
						$resp = $this->model->write('goods_photo','update', $param);
						if($resp['status']){$ok = true;}
						$this->setActionLog('goods_list_business','UPDATE','修改商企商品轮播图');
					}
				}
			}
	...
	public function delete(){
		...
		$ref = urldecode($ref);
		$param['id'] = $id;
		$resp = $this->model->write('goods_list_business','delete', $param);
		$this->setActionLog('goods_list_business','DELETE','删除商企商品');
		//删除商品搜索引擎
		try{
			if($resp['status']){
	...
	public function upstore(){
		...
		if($data[1]){
			$param['id'] = $data[0];
			$resp = $this->model->write('goods_list_business','upstore', $param);
			//新增/修改商品搜索引擎
			try{
				if($resp['status']){
					$url = SCHOOLAPI."goods/insertsearchengine";
			...
			$this->setActionLog('goods_list_business','UPDATE','更改商企商品状态');
			$this->jsonout($resp['status'],array(
	...
	public function downstore(){
		...
		$ref = urldecode($ref);
		$param['id'] = $id;
		$resp = $this->model->write('goods_list_business','downstore', $param);
		//删除商品搜索引擎
		try{
			if($resp['status']){
				$url = SCHOOLAPI."goods/deletesearchengine";
	...
	public function photoDelete(){
		...
		$this->setActionLog('goods_list_business','DELETE','删除商企商品图片');
		$this->jsonout($resp['status'],array(
			'status'=> 1,
			'msg'=>'图片删除成功',
			'ref'=> $ref
	...
	public function getpartgoods(){
	    ...
	    $this->view($data, 'goods_list_business/partgoods'); //输出
	}
  ...
  public function savesize(){
      ...
      $dataitem = $this->model->write('goods_list_business','savesize', $items);
      $ret      = $dataitem['status'];
      if ($ret)
      {
          $data['msg'] = true;
  ...
  public function savesizegoods(){
	 ...
	 $dataitem = $this->model->write('goods_list_business','savesizegoods', $items);
