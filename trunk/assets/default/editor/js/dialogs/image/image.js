...
//                }
    ...
    var Upload = {
        ...
        init: function (editor, $w) {
            ...
            me.submit();
            me.drag();
			$(".edui-image-uploadsAdd", me.dialog).on("click", function () {
				$("#upmsg").show();
				var img = document.getElementById('uploadsImg');
				var isOK = true;
				var windowURL = window.URL || window.webkitURL,
				imgs = new Image();
				imgs.src = windowURL.createObjectURL(img.files[0]);
	            imgs.onload = function () {
	            	$(".edui-image-uploadsAdd").parent().append("<img src='" + imgs.src + "' id='testimg' />");
					var testimg = document.getElementById('testimg');
					var x = testimg.width;
					testimg.remove();
					if(x>640){
						$("#upmsg").hide();
						$.alert({title: '提示',body: '图片宽度不能超过640px！'});
						isOK = false;
					}
	            }
        		...
        		imgFile.onload = function () {  
		            ...
		            imgData_1 = imgData.split(','),
					imgBaseData = imgData_1[1]; //base64数据 
		            var ext = img.value.substring(img.value.indexOf(".")+1);
		            
		            if(isOK){
		            	$.ajax({
					      type:'post',
					      url:OO._IMGUPURL,
					      data:{"fileStr": imgBaseData,"fileName": '0.'+ext,"filePath": OO._EDITIMGPATH ,"imagePath": OO._EDITIMGURL},
					      dataType:"json",
					      async: false,
					      success:function(d){
					      	$("#upmsg").hide();
					        if(1 == d.status){
					        	var url = d.data[0].photoUrl;
					        	if(url){
			...
			$(".edui-image-uploadsAdd").hover(function () {
    ...
    UM.registerWidget('image', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            ...
            "<li  class=\"edui-tab-item edui-active\"><a data-context=\".edui-image-JimgUploads\" class=\"edui-tab-text\"><%=lang_tab_local%></a></li>" +
            "<li  class=\"edui-tab-item\"><a data-context=\".edui-image-JimgSearch\" class=\"edui-tab-text\"><%=lang_tab_imgSearch%></a></li>" +
            "</ul>" +
            "<div class=\"edui-tab-content\">" +
            "<div class=\"edui-image-JimgUploads edui-tab-pane edui-active\">" +
            "<div class=\"edui-image-searchBar\">" +
            "<table><tr><td><input class=\"edui-image-file\" type=\"file\"  id=\"uploadsImg\" accept=\".bmp,.jpg,.gif,.png\"></td>" +
            "<td><div class=\"edui-image-uploadsAdd\">上传</div></td><td style=\"color:#FF0000;\">图片宽度不能超过640px！</td></tr>" +
            "<tr><td><span id=\"upmsg\" style=\"display: none;\">正在上传...<div class=\"sui-progress progress-striped active\">"+
            ...
            "<td><div class=\"edui-image-searchAdd\"><%=lang_btn_add%></div></td></tr>" +
            "<tr><td><div id=\"J_add_photo_editor\" class=\"edui-image-searchAdd\" style=\"margin-left:0px\">微信素材</div></td>" +
            "<td></td></tr>" +
            "</table>" +
            "</div>" +
