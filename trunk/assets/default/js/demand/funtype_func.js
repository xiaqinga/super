...
var Funtype = {
	// 职位列表
	init : function(){
		if($('#FuntypeIDOld').val().length>0){
			fun_flag_arr=$('#FuntypeIDOld').val().split(',');
		}
		var _str='',_id='';
		if (fun_flag_arr.length>0){
			for (var i in fun_flag_arr){
				if(typeof(fun_flag_arr[i])!='function'){
					_str+=','+fun_a[fun_flag_arr[i]];
					_id+=','+fun_flag_arr[i];
				}
			}
			$('#btn_FuntypeID').val(_str.substring(1));
			$('#positionClassCode').val(_id.substring(1));
			$('#FuntypeIDOld').val(_id.substring(1));
		}
	},
	Show : function(){
		if($('#FuntypeIDOld').val().length>0){
	...
	confirm : function(){
		...
		$('#btn_FuntypeID').attr('title', funStr);;
		$('#positionClassCode').val(fun_flag_arr);
		$('#FuntypeIDOld').val(fun_flag_arr);
		boxAlpha();
	},
