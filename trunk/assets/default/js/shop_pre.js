...
backend.photo = (function(duplication) {
    ...
    */
    ...
    duplication.mvcommon = function($d,$t,n) {

        var main_photo = main_photos[parseInt(n)];
        items_length = parseInt(backend.comomn.objectLength(main_photo));
        ...
        if($d=='left'){
          ...
          new_index = parseInt($t.parent().prev().index());
        }else if($d=='right'){
          if(index == (items_length-1)){return false;}
          new_o=$t.parent().next().children(".Js_img");
          new_index = parseInt($t.parent().next().index());
        }
        ...
        //交换对象
        old_object =  main_photo[index];
        new_object =  main_photo[new_index];
        main_photo[index] = new_object;
        main_photo[new_index] = old_object;

        //console.log(index,new_index);
        //console.log(old_object, new_object);
        
        //按对象实际排序 displayOrder
        if(typeof(main_photo[index])!='undefined'){
          main_photo[index].displayOrder = (index+1);
        }
        if(typeof(main_photo[new_index])!='undefined'){
          main_photo[new_index].displayOrder = (new_index+1);
        }
     ...
     * -------------------------------
    */
    duplication.mv = function(n) {

      $(".photo_main"+n).on('click', '.Js_item .Js_move_left', function(){
        backend.photo.mvcommon('left',$(this),n);
      })

      $(".photo_main"+n).on('click', '.Js_item .Js_move_right', function(){
        backend.photo.mvcommon('right',$(this),n);
      })
    };
...
var Size = (function(duplication) {
    ...
    */
    duplication.get_size_attr = function(o,k) {
                ...
                if(org_select_a_id && !org_select_b_id){  //仅勾选选项1的值
                    for (var i = sizeItem.A.length - 1; i >= 0; i--) {
                        ...
                        tr.innerHTML =  '<td class="js_sizes_a" style="word-break: break-all;word-wrap:break-word;">'+sizeItem.A[i].value+'</td>'+
                                        ...
                                        '<td style="word-break: break-all;word-wrap:break-word;"><input min="0" class="input-medium js_originalPrice" type="text" value=""></td>'+
                                        ...
                                        '<td style="word-break: break-all;word-wrap:break-word;">'+getdropdown('',_li)+'</td>';
        ...
        $.ajax({
            type    : "post",
            async   : false,
            url     : OO._SRVPATH+'goods_list_pre/sizeAttr',
            data    : "id=" + idata.id,
            dataType: 'json',
        ...
        okHide: function() {
          ...
          for (var i = formsizeGoods.length - 1; i >= 0; i--) {
              ...
              var sizeItems = {};
                  ...
                  sizeItems.originalPrice = backend.comomn.save_json(formsizeGoods[i].querySelector(".js_originalPrice").value);
                  sizeItems.preferentialPrice = backend.comomn.save_json(formsizeGoods[i].querySelector(".js_preferentialPrice").value);
                  sizeItems.photosId = backend.comomn.save_json(formsizeGoods[i].querySelector(".photoGroupSelect input").value);
                  ...
                  if(!sizeItems.weight) {
                      ...
                      return;
                  }else if(!sizeItems.originalPrice) {
                      $.alert("请输入市场价!");  
                      $(formsizeGoods[i].querySelector(".js_originalPrice")).trigger("focus");
                      return;
                  }else if(!sizeItems.preferentialPrice) {
                      $.alert("请输入优惠价!");  
                      $(formsizeGoods[i].querySelector(".js_preferentialPrice")).trigger("focus");
                      return;
                  }else if(!sizeItems.stockNum) {
                      $.alert("请输入库存数量!");  
                      $(formsizeGoods[i].querySelector(".js_stockNum")).trigger("focus");
                      return;
                  }else if(!sizeItems.photosId) {
                      $.alert("请选择轮播图!");
                      return;
                  }
          ...
          $.ajax({
              type    : "post",
              async   : false,
              url     : OO._SRVPATH+'goods_list_pre/savesizegoods', 
              ...
              success : function (data)
              {
                canupdate = data['msg'];
                  $.alert("保存成功");
                  SAYIMO.go_url(OO._SRVPATH+"goods_list_pre/normsedit?id="+$("#id").val());
              }
          });
    ...
    */
    duplication.saveSize = function() {
        ...
        //console.log(post_f_vals,post_s_vals);
        $.ajax({
            type    : "post",
            async   : false,
            url     : OO._SRVPATH+'goods_list_pre/savesize',
            data    : "goods_id=" + $("#id").val() + "&f_vals=" + JSON.stringify(post_f_vals) + "&s_vals=" + JSON.stringify(post_s_vals),
            dataType: 'json',
            success : function (data)
            {
                null
