;(function($){
	var LightBox = function(settings){
		...
		this.renderDOM();//渲染剩余的DOM，并且插入到body
	...
	LightBox.prototype={
		...
		changePic:function(width,height){
			...
			height= height*scale;
			if(width < 300){
				this.picViewArea.animate({width:width+100-10,height:height+100-10},self.settings.speed);
				this.popupWin.animate({width:width+100,height:height+100,marginLeft:-(width+50/2),top:(winHeight-(height+50))/2},self.settings.speed,function(){
					self.popupPic.css({width:width-10,height:height-10,margin:50}).fadeIn();
					self.picCaptionArea.css({"margin-top": -(self.picCaptionArea.height()+5) + "px"}).fadeIn();
					self.flag = true;
					self.clear = true;
				});
			}else{
...
$(function(){var lightbox = new LightBox({speed:400});});