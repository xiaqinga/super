	...
	$.fn.J_jcorp = function(setting){
		...
		function Jscale(){//图片缩放
			...
			scale.prototype = {
				...
				ondrag : function (pos){
					var f = this;					
					    w = img_w * pos - 10 < 40 ? 40 : img_w * pos - 10,
						h = img_h * pos - 10 < 40 ? 40 : img_h * pos - 10;	
					if(f.flag == false){

						jcrop_api.release();
						jcrop_api.disable();//移除jcorp
		...
		function loadPic(w,h){//渲染图片
			...
			setJcorpapi(img_w,img_h);
		}
		
		function setJcorpapi(select_w,select_h){//开启jcorp插件
			...
			$('#' + imgId).Jcrop({
				...
				jcrop_api.setSelect([(select_w- setting.picSize[0])/2,(select_h- setting.picSize[1])/2,(select_w+ setting.picSize[0])/2,(select_h- setting.picSize[1])/2]);
		...
		function Jsave(){//保存
			var Jsave_w,Jsave_h
			if(setting.picSize[0] == '0' && setting.picSize[1] == '0'){
				Jsave_w = J_y_img.find('img')[0].naturalWidth;
				Jsave_h = J_y_img.find('img')[0].naturalHeight;

				setting.picSize[0] = Jsave_w;
				setting.picSize[1] = Jsave_h;

				J_canvas.width = setting.picSize[0];
				J_canvas.height = setting.picSize[1];

			}else{
				J_canvas.width = setting.picSize[0];//设置canvas宽
				J_canvas.height = setting.picSize[1];//设置canvas高	
			}			
			var img = new Image(),
				ctx = J_canvas.getContext("2d");
			img.src = $("#" + imgId).find('img').attr('src');
			img.onload = function(){
				ctx.drawImage(img, 0, 0, setting.picSize[0], setting.picSize[1]);
		...
		function postAjax(s){
			...
			$.ajax({
				...
				type: "POST",
				data: {"fileStr": s_1,"fileName": '0.jpg',"filePath": setting.filePath ,"imagePath": setting.imagePath},
				jsonp:"callback",
